...
docker container exec boss /bin/bash -c "echo \"sed -i '45,46c path_remove CMTPATH  \\\"\\\${WorkArea}\\\"\npath_prepend CMTPATH \\\"\\\${WorkArea}\\\"' requirements\"| sudo tee -a /home/setup708.sh"
...
docker container exec boss /bin/bash -c 'echo "cd ../run&&boss.exe jobOptions_sim.txt"  | sudo tee -a /home/setup708.sh'
docker container exec boss /bin/bash -c 'echo "boss.exe jobOptions_rec.txt"  | sudo tee -a /home/setup708.sh'
docker container exec boss /bin/bash -c 'echo "boss.exe jobOptions_ana_rhopi.txt"  | sudo tee -a /home/setup708.sh'
