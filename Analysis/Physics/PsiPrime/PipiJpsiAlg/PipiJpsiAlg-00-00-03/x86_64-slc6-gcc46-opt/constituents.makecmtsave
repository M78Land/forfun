...
#cmt_local_tagfile = $(PipiJpsiAlg_tag).make
cmt_local_tagfile = $(bin)$(PipiJpsiAlg_tag).make
...
cmt_local_setup_PipiJpsiAlg = $(bin)setup_PipiJpsiAlg$$$$.make
...
PipiJpsiAlg_extratags = -tag_add=target_PipiJpsiAlg

#$(cmt_local_tagfile_PipiJpsiAlg) : $(cmt_lock_setup)
ifndef QUICK
$(cmt_local_tagfile_PipiJpsiAlg) ::
else
$(cmt_local_tagfile_PipiJpsiAlg) :
endif
	$(echo) "(constituents.make) Rebuilding $@"; \
	  if test -f $(cmt_local_tagfile_PipiJpsiAlg); then /bin/rm -f $(cmt_local_tagfile_PipiJpsiAlg); fi ; \
	  $(cmtexe) -tag=$(tags) $(PipiJpsiAlg_extratags) build tag_makefile >>$(cmt_local_tagfile_PipiJpsiAlg)
	$(echo) "(constituents.make) Rebuilding $(cmt_final_setup_PipiJpsiAlg)"; \
	  test ! -f $(cmt_local_setup_PipiJpsiAlg) || \rm -f $(cmt_local_setup_PipiJpsiAlg); \
	  trap '\rm -f $(cmt_local_setup_PipiJpsiAlg)' 0 1 2 15; \
	  $(cmtexe) -tag=$(tags) $(PipiJpsiAlg_extratags) show setup >$(cmt_local_setup_PipiJpsiAlg) && \
	  if [ -f $(cmt_final_setup_PipiJpsiAlg) ] && \
	    \cmp -s $(cmt_final_setup_PipiJpsiAlg) $(cmt_local_setup_PipiJpsiAlg); then \
	    \rm $(cmt_local_setup_PipiJpsiAlg); else \
	    \mv -f $(cmt_local_setup_PipiJpsiAlg) $(cmt_final_setup_PipiJpsiAlg); fi
...
not_PipiJpsiAlg_dependencies = { n=0; for p in $?; do m=0; for d in $(PipiJpsiAlg_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PipiJpsiAlgdirs :
	@if test ! -d $(bin)PipiJpsiAlg; then $(mkdir) -p $(bin)PipiJpsiAlg; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PipiJpsiAlg
else
PipiJpsiAlgdirs : ;
endif
...
#	$(echo) "STRUCTURED_OUTPUT="$(bin)PipiJpsiAlg
#
#$(cmt_local_PipiJpsiAlg_makefile) :: $(PipiJpsiAlg_dependencies) $(cmt_local_tagfile_PipiJpsiAlg) build_library_links dirs PipiJpsiAlgdirs
#else
#$(cmt_local_PipiJpsiAlg_makefile) :: $(PipiJpsiAlg_dependencies) $(cmt_local_tagfile_PipiJpsiAlg) build_library_links dirs
#endif
#else
#$(cmt_local_PipiJpsiAlg_makefile) :: $(cmt_local_tagfile_PipiJpsiAlg)
#endif

ifdef cmt_PipiJpsiAlg_has_target_tag

ifndef QUICK
$(cmt_local_PipiJpsiAlg_makefile) : $(PipiJpsiAlg_dependencies) build_library_links
	$(echo) "(constituents.make) Building PipiJpsiAlg.make"; \
	  $(cmtexe) -tag=$(tags) $(PipiJpsiAlg_extratags) build constituent_config -out=$(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlg
else
$(cmt_local_PipiJpsiAlg_makefile) : $(PipiJpsiAlg_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PipiJpsiAlg) ] || \
	  [ ! -f $(cmt_final_setup_PipiJpsiAlg) ] || \
	  $(not_PipiJpsiAlg_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PipiJpsiAlg.make"; \
	  $(cmtexe) -tag=$(tags) $(PipiJpsiAlg_extratags) build constituent_config -out=$(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlg; \
	  fi
endif
...
$(cmt_local_PipiJpsiAlg_makefile) : $(PipiJpsiAlg_dependencies) build_library_links
	$(echo) "(constituents.make) Building PipiJpsiAlg.make"; \
	  $(cmtexe) -f=$(bin)PipiJpsiAlg.in -tag=$(tags) $(PipiJpsiAlg_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlg
else
$(cmt_local_PipiJpsiAlg_makefile) : $(PipiJpsiAlg_dependencies) $(cmt_build_library_linksstamp) $(bin)PipiJpsiAlg.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PipiJpsiAlg) ] || \
	  [ ! -f $(cmt_final_setup_PipiJpsiAlg) ] || \
	  $(not_PipiJpsiAlg_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PipiJpsiAlg_extratags) build constituent_makefile -out=$(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlg

PipiJpsiAlg :: $(PipiJpsiAlg_dependencies) $(cmt_local_PipiJpsiAlg_makefile) dirs PipiJpsiAlgdirs
	$(echo) "(constituents.make) Starting PipiJpsiAlg"
	@if test -f $(cmt_local_PipiJpsiAlg_makefile); then \
	  $(MAKE) -f $(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlg; \
	  fi
#	@$(MAKE) -f $(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlg
	$(echo) "(constituents.make) PipiJpsiAlg done"

clean :: PipiJpsiAlgclean

PipiJpsiAlgclean :: $(PipiJpsiAlgclean_dependencies) ##$(cmt_local_PipiJpsiAlg_makefile)
	$(echo) "(constituents.make) Starting PipiJpsiAlgclean"
	@-if test -f $(cmt_local_PipiJpsiAlg_makefile); then \
	  $(MAKE) -f $(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlgclean; \
	fi
	$(echo) "(constituents.make) PipiJpsiAlgclean done"
#	@-$(MAKE) -f $(cmt_local_PipiJpsiAlg_makefile) PipiJpsiAlgclean

##	  /bin/rm -f $(cmt_local_PipiJpsiAlg_makefile) $(bin)PipiJpsiAlg_dependencies.make

install :: PipiJpsiAlginstall

PipiJpsiAlginstall :: $(PipiJpsiAlg_dependencies) $(cmt_local_PipiJpsiAlg_makefile)
	$(echo) "(constituents.make) Starting install PipiJpsiAlg"
	@-$(MAKE) -f $(cmt_local_PipiJpsiAlg_makefile) install
	$(echo) "(constituents.make) install PipiJpsiAlg done"

uninstall : PipiJpsiAlguninstall

$(foreach d,$(PipiJpsiAlg_dependencies),$(eval $(d)uninstall_dependencies += PipiJpsiAlguninstall))

PipiJpsiAlguninstall : $(PipiJpsiAlguninstall_dependencies) ##$(cmt_local_PipiJpsiAlg_makefile)
	$(echo) "(constituents.make) Starting uninstall PipiJpsiAlg"
	@if test -f $(cmt_local_PipiJpsiAlg_makefile); then \
	  $(MAKE) -f $(cmt_local_PipiJpsiAlg_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PipiJpsiAlg_makefile) uninstall
	$(echo) "(constituents.make) uninstall PipiJpsiAlg done"

remove_library_links :: PipiJpsiAlguninstall
...
#cmt_local_tagfile_config = $(PipiJpsiAlg_tag)_config.make
cmt_local_tagfile_config = $(bin)$(PipiJpsiAlg_tag)_config.make
cmt_local_setup_config = $(bin)setup_config$$$$.make
cmt_final_setup_config = $(bin)setup_config.make
#cmt_final_setup_config = $(bin)PipiJpsiAlg_configsetup.make
...
#cmt_local_tagfile_config = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_config = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_config = $(bin)setup.make
#cmt_final_setup_config = $(bin)PipiJpsiAlgsetup.make
cmt_local_config_makefile = $(bin)config.make
...
#cmt_local_tagfile_install_includes = $(PipiJpsiAlg_tag)_install_includes.make
cmt_local_tagfile_install_includes = $(bin)$(PipiJpsiAlg_tag)_install_includes.make
...
#cmt_final_setup_install_includes = $(bin)PipiJpsiAlg_install_includessetup.make
...
#cmt_local_tagfile_install_includes = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_install_includes = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_install_includes = $(bin)setup.make
#cmt_final_setup_install_includes = $(bin)PipiJpsiAlgsetup.make
cmt_local_install_includes_makefile = $(bin)install_includes.make
...
#cmt_local_tagfile_make = $(PipiJpsiAlg_tag)_make.make
cmt_local_tagfile_make = $(bin)$(PipiJpsiAlg_tag)_make.make
cmt_local_setup_make = $(bin)setup_make$$$$.make
cmt_final_setup_make = $(bin)setup_make.make
#cmt_final_setup_make = $(bin)PipiJpsiAlg_makesetup.make
...
#cmt_local_tagfile_make = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_make = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_make = $(bin)setup.make
#cmt_final_setup_make = $(bin)PipiJpsiAlgsetup.make
cmt_local_make_makefile = $(bin)make.make
...
#cmt_local_tagfile_CompilePython = $(PipiJpsiAlg_tag)_CompilePython.make
cmt_local_tagfile_CompilePython = $(bin)$(PipiJpsiAlg_tag)_CompilePython.make
cmt_local_setup_CompilePython = $(bin)setup_CompilePython$$$$.make
cmt_final_setup_CompilePython = $(bin)setup_CompilePython.make
#cmt_final_setup_CompilePython = $(bin)PipiJpsiAlg_CompilePythonsetup.make
...
#cmt_local_tagfile_CompilePython = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_CompilePython = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_CompilePython = $(bin)setup.make
#cmt_final_setup_CompilePython = $(bin)PipiJpsiAlgsetup.make
cmt_local_CompilePython_makefile = $(bin)CompilePython.make
...
#cmt_local_tagfile_qmtest_run = $(PipiJpsiAlg_tag)_qmtest_run.make
cmt_local_tagfile_qmtest_run = $(bin)$(PipiJpsiAlg_tag)_qmtest_run.make
cmt_local_setup_qmtest_run = $(bin)setup_qmtest_run$$$$.make
cmt_final_setup_qmtest_run = $(bin)setup_qmtest_run.make
#cmt_final_setup_qmtest_run = $(bin)PipiJpsiAlg_qmtest_runsetup.make
...
#cmt_local_tagfile_qmtest_run = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_qmtest_run = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_qmtest_run = $(bin)setup.make
#cmt_final_setup_qmtest_run = $(bin)PipiJpsiAlgsetup.make
cmt_local_qmtest_run_makefile = $(bin)qmtest_run.make
...
#cmt_local_tagfile_qmtest_summarize = $(PipiJpsiAlg_tag)_qmtest_summarize.make
cmt_local_tagfile_qmtest_summarize = $(bin)$(PipiJpsiAlg_tag)_qmtest_summarize.make
...
#cmt_final_setup_qmtest_summarize = $(bin)PipiJpsiAlg_qmtest_summarizesetup.make
...
#cmt_local_tagfile_qmtest_summarize = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_qmtest_summarize = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_qmtest_summarize = $(bin)setup.make
#cmt_final_setup_qmtest_summarize = $(bin)PipiJpsiAlgsetup.make
cmt_local_qmtest_summarize_makefile = $(bin)qmtest_summarize.make
...
#cmt_local_tagfile_TestPackage = $(PipiJpsiAlg_tag)_TestPackage.make
cmt_local_tagfile_TestPackage = $(bin)$(PipiJpsiAlg_tag)_TestPackage.make
cmt_local_setup_TestPackage = $(bin)setup_TestPackage$$$$.make
cmt_final_setup_TestPackage = $(bin)setup_TestPackage.make
#cmt_final_setup_TestPackage = $(bin)PipiJpsiAlg_TestPackagesetup.make
...
#cmt_local_tagfile_TestPackage = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_TestPackage = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_TestPackage = $(bin)setup.make
#cmt_final_setup_TestPackage = $(bin)PipiJpsiAlgsetup.make
cmt_local_TestPackage_makefile = $(bin)TestPackage.make
...
#cmt_local_tagfile_TestProject = $(PipiJpsiAlg_tag)_TestProject.make
cmt_local_tagfile_TestProject = $(bin)$(PipiJpsiAlg_tag)_TestProject.make
cmt_local_setup_TestProject = $(bin)setup_TestProject$$$$.make
cmt_final_setup_TestProject = $(bin)setup_TestProject.make
#cmt_final_setup_TestProject = $(bin)PipiJpsiAlg_TestProjectsetup.make
...
#cmt_local_tagfile_TestProject = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_TestProject = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_TestProject = $(bin)setup.make
#cmt_final_setup_TestProject = $(bin)PipiJpsiAlgsetup.make
cmt_local_TestProject_makefile = $(bin)TestProject.make
...
#cmt_local_tagfile_new_rootsys = $(PipiJpsiAlg_tag)_new_rootsys.make
cmt_local_tagfile_new_rootsys = $(bin)$(PipiJpsiAlg_tag)_new_rootsys.make
cmt_local_setup_new_rootsys = $(bin)setup_new_rootsys$$$$.make
cmt_final_setup_new_rootsys = $(bin)setup_new_rootsys.make
#cmt_final_setup_new_rootsys = $(bin)PipiJpsiAlg_new_rootsyssetup.make
...
#cmt_local_tagfile_new_rootsys = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_new_rootsys = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_new_rootsys = $(bin)setup.make
#cmt_final_setup_new_rootsys = $(bin)PipiJpsiAlgsetup.make
cmt_local_new_rootsys_makefile = $(bin)new_rootsys.make
...
#cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(PipiJpsiAlg_tag)_DatabaseSvc_check_install_scripts.make
cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(bin)$(PipiJpsiAlg_tag)_DatabaseSvc_check_install_scripts.make
...
#cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)PipiJpsiAlg_DatabaseSvc_check_install_scriptssetup.make
...
#cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(PipiJpsiAlg_tag).make
cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(bin)$(PipiJpsiAlg_tag).make
cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)setup.make
#cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)PipiJpsiAlgsetup.make
cmt_local_DatabaseSvc_check_install_scripts_makefile = $(bin)DatabaseSvc_check_install_scripts.make
