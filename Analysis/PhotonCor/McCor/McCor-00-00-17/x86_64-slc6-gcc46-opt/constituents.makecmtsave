...
#cmt_local_tagfile = $(McCor_tag).make
cmt_local_tagfile = $(bin)$(McCor_tag).make
...
cmt_local_setup_McCor = $(bin)setup_McCor$$$$.make
...
McCor_extratags = -tag_add=target_McCor

#$(cmt_local_tagfile_McCor) : $(cmt_lock_setup)
ifndef QUICK
$(cmt_local_tagfile_McCor) ::
else
$(cmt_local_tagfile_McCor) :
endif
	$(echo) "(constituents.make) Rebuilding $@"; \
	  if test -f $(cmt_local_tagfile_McCor); then /bin/rm -f $(cmt_local_tagfile_McCor); fi ; \
	  $(cmtexe) -tag=$(tags) $(McCor_extratags) build tag_makefile >>$(cmt_local_tagfile_McCor)
	$(echo) "(constituents.make) Rebuilding $(cmt_final_setup_McCor)"; \
	  test ! -f $(cmt_local_setup_McCor) || \rm -f $(cmt_local_setup_McCor); \
	  trap '\rm -f $(cmt_local_setup_McCor)' 0 1 2 15; \
	  $(cmtexe) -tag=$(tags) $(McCor_extratags) show setup >$(cmt_local_setup_McCor) && \
	  if [ -f $(cmt_final_setup_McCor) ] && \
	    \cmp -s $(cmt_final_setup_McCor) $(cmt_local_setup_McCor); then \
	    \rm $(cmt_local_setup_McCor); else \
	    \mv -f $(cmt_local_setup_McCor) $(cmt_final_setup_McCor); fi
...
not_McCor_dependencies = { n=0; for p in $?; do m=0; for d in $(McCor_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
McCordirs :
	@if test ! -d $(bin)McCor; then $(mkdir) -p $(bin)McCor; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)McCor
else
McCordirs : ;
endif
...
#	$(echo) "STRUCTURED_OUTPUT="$(bin)McCor
#
#$(cmt_local_McCor_makefile) :: $(McCor_dependencies) $(cmt_local_tagfile_McCor) build_library_links dirs McCordirs
#else
#$(cmt_local_McCor_makefile) :: $(McCor_dependencies) $(cmt_local_tagfile_McCor) build_library_links dirs
#endif
#else
#$(cmt_local_McCor_makefile) :: $(cmt_local_tagfile_McCor)
#endif

ifdef cmt_McCor_has_target_tag

ifndef QUICK
$(cmt_local_McCor_makefile) : $(McCor_dependencies) build_library_links
	$(echo) "(constituents.make) Building McCor.make"; \
	  $(cmtexe) -tag=$(tags) $(McCor_extratags) build constituent_config -out=$(cmt_local_McCor_makefile) McCor
else
$(cmt_local_McCor_makefile) : $(McCor_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_McCor) ] || \
	  [ ! -f $(cmt_final_setup_McCor) ] || \
	  $(not_McCor_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building McCor.make"; \
	  $(cmtexe) -tag=$(tags) $(McCor_extratags) build constituent_config -out=$(cmt_local_McCor_makefile) McCor; \
	  fi
endif
...
$(cmt_local_McCor_makefile) : $(McCor_dependencies) build_library_links
	$(echo) "(constituents.make) Building McCor.make"; \
	  $(cmtexe) -f=$(bin)McCor.in -tag=$(tags) $(McCor_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_McCor_makefile) McCor
else
$(cmt_local_McCor_makefile) : $(McCor_dependencies) $(cmt_build_library_linksstamp) $(bin)McCor.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_McCor) ] || \
	  [ ! -f $(cmt_final_setup_McCor) ] || \
	  $(not_McCor_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(McCor_extratags) build constituent_makefile -out=$(cmt_local_McCor_makefile) McCor

McCor :: $(McCor_dependencies) $(cmt_local_McCor_makefile) dirs McCordirs
	$(echo) "(constituents.make) Starting McCor"
	@if test -f $(cmt_local_McCor_makefile); then \
	  $(MAKE) -f $(cmt_local_McCor_makefile) McCor; \
	  fi
#	@$(MAKE) -f $(cmt_local_McCor_makefile) McCor
	$(echo) "(constituents.make) McCor done"

clean :: McCorclean

McCorclean :: $(McCorclean_dependencies) ##$(cmt_local_McCor_makefile)
	$(echo) "(constituents.make) Starting McCorclean"
	@-if test -f $(cmt_local_McCor_makefile); then \
	  $(MAKE) -f $(cmt_local_McCor_makefile) McCorclean; \
	fi
	$(echo) "(constituents.make) McCorclean done"
#	@-$(MAKE) -f $(cmt_local_McCor_makefile) McCorclean

##	  /bin/rm -f $(cmt_local_McCor_makefile) $(bin)McCor_dependencies.make

install :: McCorinstall

McCorinstall :: $(McCor_dependencies) $(cmt_local_McCor_makefile)
	$(echo) "(constituents.make) Starting install McCor"
	@-$(MAKE) -f $(cmt_local_McCor_makefile) install
	$(echo) "(constituents.make) install McCor done"

uninstall : McCoruninstall

$(foreach d,$(McCor_dependencies),$(eval $(d)uninstall_dependencies += McCoruninstall))

McCoruninstall : $(McCoruninstall_dependencies) ##$(cmt_local_McCor_makefile)
	$(echo) "(constituents.make) Starting uninstall McCor"
	@if test -f $(cmt_local_McCor_makefile); then \
	  $(MAKE) -f $(cmt_local_McCor_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_McCor_makefile) uninstall
	$(echo) "(constituents.make) uninstall McCor done"

remove_library_links :: McCoruninstall
...
#cmt_local_tagfile_config = $(McCor_tag)_config.make
cmt_local_tagfile_config = $(bin)$(McCor_tag)_config.make
cmt_local_setup_config = $(bin)setup_config$$$$.make
cmt_final_setup_config = $(bin)setup_config.make
#cmt_final_setup_config = $(bin)McCor_configsetup.make
...
#cmt_local_tagfile_config = $(McCor_tag).make
cmt_local_tagfile_config = $(bin)$(McCor_tag).make
cmt_final_setup_config = $(bin)setup.make
#cmt_final_setup_config = $(bin)McCorsetup.make
cmt_local_config_makefile = $(bin)config.make
...
#cmt_local_tagfile_install_includes = $(McCor_tag)_install_includes.make
cmt_local_tagfile_install_includes = $(bin)$(McCor_tag)_install_includes.make
...
#cmt_final_setup_install_includes = $(bin)McCor_install_includessetup.make
...
#cmt_local_tagfile_install_includes = $(McCor_tag).make
cmt_local_tagfile_install_includes = $(bin)$(McCor_tag).make
cmt_final_setup_install_includes = $(bin)setup.make
#cmt_final_setup_install_includes = $(bin)McCorsetup.make
cmt_local_install_includes_makefile = $(bin)install_includes.make
...
#cmt_local_tagfile_make = $(McCor_tag)_make.make
cmt_local_tagfile_make = $(bin)$(McCor_tag)_make.make
cmt_local_setup_make = $(bin)setup_make$$$$.make
cmt_final_setup_make = $(bin)setup_make.make
#cmt_final_setup_make = $(bin)McCor_makesetup.make
...
#cmt_local_tagfile_make = $(McCor_tag).make
cmt_local_tagfile_make = $(bin)$(McCor_tag).make
cmt_final_setup_make = $(bin)setup.make
#cmt_final_setup_make = $(bin)McCorsetup.make
cmt_local_make_makefile = $(bin)make.make
...
#cmt_local_tagfile_CompilePython = $(McCor_tag)_CompilePython.make
cmt_local_tagfile_CompilePython = $(bin)$(McCor_tag)_CompilePython.make
cmt_local_setup_CompilePython = $(bin)setup_CompilePython$$$$.make
cmt_final_setup_CompilePython = $(bin)setup_CompilePython.make
#cmt_final_setup_CompilePython = $(bin)McCor_CompilePythonsetup.make
...
#cmt_local_tagfile_CompilePython = $(McCor_tag).make
cmt_local_tagfile_CompilePython = $(bin)$(McCor_tag).make
cmt_final_setup_CompilePython = $(bin)setup.make
#cmt_final_setup_CompilePython = $(bin)McCorsetup.make
cmt_local_CompilePython_makefile = $(bin)CompilePython.make
...
#cmt_local_tagfile_qmtest_run = $(McCor_tag)_qmtest_run.make
cmt_local_tagfile_qmtest_run = $(bin)$(McCor_tag)_qmtest_run.make
cmt_local_setup_qmtest_run = $(bin)setup_qmtest_run$$$$.make
cmt_final_setup_qmtest_run = $(bin)setup_qmtest_run.make
#cmt_final_setup_qmtest_run = $(bin)McCor_qmtest_runsetup.make
...
#cmt_local_tagfile_qmtest_run = $(McCor_tag).make
cmt_local_tagfile_qmtest_run = $(bin)$(McCor_tag).make
cmt_final_setup_qmtest_run = $(bin)setup.make
#cmt_final_setup_qmtest_run = $(bin)McCorsetup.make
cmt_local_qmtest_run_makefile = $(bin)qmtest_run.make
...
#cmt_local_tagfile_qmtest_summarize = $(McCor_tag)_qmtest_summarize.make
cmt_local_tagfile_qmtest_summarize = $(bin)$(McCor_tag)_qmtest_summarize.make
...
#cmt_final_setup_qmtest_summarize = $(bin)McCor_qmtest_summarizesetup.make
...
#cmt_local_tagfile_qmtest_summarize = $(McCor_tag).make
cmt_local_tagfile_qmtest_summarize = $(bin)$(McCor_tag).make
cmt_final_setup_qmtest_summarize = $(bin)setup.make
#cmt_final_setup_qmtest_summarize = $(bin)McCorsetup.make
cmt_local_qmtest_summarize_makefile = $(bin)qmtest_summarize.make
...
#cmt_local_tagfile_TestPackage = $(McCor_tag)_TestPackage.make
cmt_local_tagfile_TestPackage = $(bin)$(McCor_tag)_TestPackage.make
cmt_local_setup_TestPackage = $(bin)setup_TestPackage$$$$.make
cmt_final_setup_TestPackage = $(bin)setup_TestPackage.make
#cmt_final_setup_TestPackage = $(bin)McCor_TestPackagesetup.make
...
#cmt_local_tagfile_TestPackage = $(McCor_tag).make
cmt_local_tagfile_TestPackage = $(bin)$(McCor_tag).make
cmt_final_setup_TestPackage = $(bin)setup.make
#cmt_final_setup_TestPackage = $(bin)McCorsetup.make
cmt_local_TestPackage_makefile = $(bin)TestPackage.make
...
#cmt_local_tagfile_TestProject = $(McCor_tag)_TestProject.make
cmt_local_tagfile_TestProject = $(bin)$(McCor_tag)_TestProject.make
cmt_local_setup_TestProject = $(bin)setup_TestProject$$$$.make
cmt_final_setup_TestProject = $(bin)setup_TestProject.make
#cmt_final_setup_TestProject = $(bin)McCor_TestProjectsetup.make
...
#cmt_local_tagfile_TestProject = $(McCor_tag).make
cmt_local_tagfile_TestProject = $(bin)$(McCor_tag).make
cmt_final_setup_TestProject = $(bin)setup.make
#cmt_final_setup_TestProject = $(bin)McCorsetup.make
cmt_local_TestProject_makefile = $(bin)TestProject.make
...
#cmt_local_tagfile_new_rootsys = $(McCor_tag)_new_rootsys.make
cmt_local_tagfile_new_rootsys = $(bin)$(McCor_tag)_new_rootsys.make
cmt_local_setup_new_rootsys = $(bin)setup_new_rootsys$$$$.make
cmt_final_setup_new_rootsys = $(bin)setup_new_rootsys.make
#cmt_final_setup_new_rootsys = $(bin)McCor_new_rootsyssetup.make
...
#cmt_local_tagfile_new_rootsys = $(McCor_tag).make
cmt_local_tagfile_new_rootsys = $(bin)$(McCor_tag).make
cmt_final_setup_new_rootsys = $(bin)setup.make
#cmt_final_setup_new_rootsys = $(bin)McCorsetup.make
cmt_local_new_rootsys_makefile = $(bin)new_rootsys.make
...
#cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(McCor_tag)_DatabaseSvc_check_install_scripts.make
cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(bin)$(McCor_tag)_DatabaseSvc_check_install_scripts.make
...
#cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)McCor_DatabaseSvc_check_install_scriptssetup.make
...
#cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(McCor_tag).make
cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(bin)$(McCor_tag).make
cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)setup.make
#cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)McCorsetup.make
cmt_local_DatabaseSvc_check_install_scripts_makefile = $(bin)DatabaseSvc_check_install_scripts.make
