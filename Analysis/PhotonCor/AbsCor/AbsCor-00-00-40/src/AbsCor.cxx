...
#include <fstream>
#include <strstream>
#include "EmcShEnCalibSvc/EmcShEnCalibSvc.h"
...
StatusCode AbsCor::execute() {
 ...
 //// by liucx
  ...
  unsigned int runFrom=m_EmcShEnCalibSvc -> getRunFrom();
  unsigned int runTo=m_EmcShEnCalibSvc -> getRunTo();
  if (!m_ReadFile) {
    runFrom0 = runFrom;
    runTo0 = runTo;
  }

  if (runNum>=runFrom0&&runNum<=runTo0&&m_ReadFile==true){

    //for test
    //cout<<"do not open the CorFunctionPar File repeatly"<<endl;
    //cout<<"runNo,RunFrom,runTo="<<runNo<<","<<runFrom<<","<<runTo<<endl;
    } else {
    if (runFrom0!=runFrom) runFrom0 = runFrom;
    if (runTo0!=runTo) runTo0 = runTo; 
...
{
  if(theid<0||theid>27){ 
    cout<<"in AbsCor EcorrFunctionMC error::thetaId is out of the region [0,27]"<<endl;
  }
  double Energy5x5=eg;
  double x=Energy5x5;
